...
import {
  FILECOIN_DEV_SUMMIT_ETH_BRUSSELS_2024_HERO_CONTENT,
  FIL_DEV_SUMMIT_ETH_BRUSSELS_2024_NAVIGATION_CONTENT,
  FILECOIN_DEV_SUMMIT_ETH_BRUSSELS_2024_PAGE_CONTENT,
  FIL_DEV_SUMMIT_PAGE_STYLE_CONTENT,
  FOOTER_FILECOIN_DEV_SUMMIT_CONTENT,
} from '@root/content/fildevsummit/filecoin-dev-eth-brussels-2024';
...
export async function generateMetadata({ params, searchParams }) {
  const title = 'FIL Dev Summit 2024: ETH Brussels';
  const description =
...
export default async function Page(props) {
  const blocks = FILECOIN_DEV_SUMMIT_ETH_BRUSSELS_2024_PAGE_CONTENT;
  const currentHeaders = headers();
  const footerContent = FOOTER_FILECOIN_DEV_SUMMIT_CONTENT;
  const hero = FILECOIN_DEV_SUMMIT_ETH_BRUSSELS_2024_HERO_CONTENT;
  const host = currentHeaders.get('host');
  const navContent = FIL_DEV_SUMMIT_ETH_BRUSSELS_2024_NAVIGATION_CONTENT;
